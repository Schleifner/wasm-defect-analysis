...
2. **Discover without editing upstream.** Give the agent the run's `prompt.md`; it fills the formatted `report.md` and stores minimal inputs or focused logs in `artifacts/`.
...
See [docs/CN-workflow.md](docs/CN-workflow.md) for the current step-by-step operating procedure, [docs/methodology.md](docs/methodology.md) for its design rationale, and [benchmarks/README.md](benchmarks/README.md) for split and leakage rules.
...
[work.md](work.md) is the original report and remains the human-readable source of truth. [benchmarks/round-001/oracle.json](benchmarks/round-001/oracle.json) preserves its eight judgments in machine-readable form:
...
```text
benchmarks/       Human-labelled supervision and future frozen eval tasks
...
runs/             Per-run prompt snapshots, reports, and artifacts
scripts/          Run creation and upstream updates
upstream/         Pinned compiler submodules
work.md           Original round-one report and human comments
```
