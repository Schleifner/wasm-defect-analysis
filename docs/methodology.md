...
The central hypothesis after round 001 is:

> Requiring evidence for supported-input reachability, conflict with design intent, and an executable semantic oracle will preserve real parser and miscompilation findings while suppressing synthetic IR and intended-behavior reports.
...
Record `yes`, `no`, or `unknown`; never silently equate code reach with a triggered fault.
...
| `reject` | `intended_behavior` | Behavior matches an explicit product policy | Document if unclear |
| `reject` | `unreachable_producer_contract` | Synthetic state cannot be produced at the relevant supported boundary | Keep as a negative example |
| `reject` | `non_actionable_diagnostic` | Cosmetic or duplicate reporting without meaningful impact | Ignore unless requirements change |
| `defer` | `insufficient_evidence` | A decisive check is unavailable or inconclusive | Record the missing experiment |

Severity is not confidence. A high-confidence cosmetic issue remains low value; a potentially severe issue without baseline evidence remains deferred.

## Iterating Prompts Without Leakage

1. Freeze scope, target commits, model settings, budget, and prompt before a run.
2. Use stable candidate IDs and retain every rejected/deferred hypothesis.
...
- [Magma](https://hexhive.epfl.ch/magma/docs/technical.html): ground-truth canaries distinguish a bug location being **reached** from its fault condition being **triggered**. Applied here as separate producer reachability and observed oracle fields.
- [Semgrep rule testing](https://semgrep.dev/docs/writing-rules/testing-rules): positive and negative fixtures make both detection and false-positive behavior explicit. Applied here by retaining rejected producer-contract cases alongside accepted examples.
- [OSS-Fuzz ideal integration](https://google.github.io/oss-fuzz/advanced-topics/ideal-integration/): checked-in targets, coverage-oriented seed corpora, reproducible testcases, and promotion of found failures into regression tests. Applied here through minimal artifacts and permanent benchmark cases.
...
A human-confirmed finding should move from a run into a permanent benchmark only after:

1. baseline behavior is reproduced at the recorded commit;
2. the supported input boundary is established;
3. the reproducer is minimized and stored or deterministically generated;
